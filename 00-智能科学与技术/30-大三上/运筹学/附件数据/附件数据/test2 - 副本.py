...
HoardCost = 0.1      # 单位货物在中转站的囤积成本（调整后）
...
total_dispatch_cost = quicksum(DispatchCost1[j - 3] * x[1, j, t] for j in P1 for t in range(timeInterval)) + \
                       quicksum(DispatchCost2[j - 11] * x[2, j, t] for j in P2 for t in range(timeInterval))
...
# 8. 剩余负载成本：假设 li 的单位成本为 0.05
total_li_cost = quicksum(0.05 * li[j, t]
                         for j in P
                         for t in range(timeInterval))
