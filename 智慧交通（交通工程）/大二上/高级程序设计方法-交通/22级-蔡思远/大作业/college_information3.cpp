...
// 派生类：教师兼行政人员
class TeacherAdministrator : public CollegePerson {
public:
    TeacherAdministrator(int id, string name, char gender, int age, string teacherDepartment, string major, string title, string adminPoliticalAffiliation, string adminJobTitle)
        : CollegePerson(id, name, gender, age), teacherDepartment(teacherDepartment), major(major), title(title), adminPoliticalAffiliation(adminPoliticalAffiliation), adminJobTitle(adminJobTitle) {}

    void display() const override {
        CollegePerson::display();
        cout << ", 教师系部: " << teacherDepartment
            << ", 教师专业: " << major
            << ", 教师职称: " << title
            << ", 行政政治面貌: " << adminPoliticalAffiliation
            << ", 行政职称: " << adminJobTitle << endl;
    }
...
private:
    ...
    string major;
    string title;
    string adminPoliticalAffiliation;
...
void addCollegePerson() {
    char choice;
    cout << "请选择要添加的人员类别 (T: 教师, L: 实验员, A: 行政人员, G: 教师兼行政人员, Q: 返回主菜单): ";
    cin >> choice;
    ...
    switch (choice) {
        ...
        case 'g':
            ...
            cout << "请输入行政职称: ";
            cin >> jobTitle;
            collegePeople.push_back(new TeacherAdministrator(id, name, gender, age, department, major, title, politicalAffiliation, jobTitle));
            break;
        default:
            cout << "无效的选项" << endl;
            break;
...
void countCollegePeople() {
    int teacherCount = 0, labAssistantCount = 0, administratorCount = 0, teacherAdminCount = 0, totalCount = 0;
    ...
    for (const auto& person : collegePeople) {
        if (dynamic_cast<Teacher*>(person)) {
            ...
            administratorCount++;
        } else if (dynamic_cast<TeacherAdministrator*>(person)) {
            teacherAdminCount++;
        }
    ...
    cout << "教师兼行政人员数量: " << teacherAdminCount << endl;
