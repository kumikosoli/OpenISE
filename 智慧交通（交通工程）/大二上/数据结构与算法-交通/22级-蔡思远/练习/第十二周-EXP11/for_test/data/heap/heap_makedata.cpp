	...
	for (i=0;i<10;i++){
		...
		for (k=0;k<m;k++){
			...
			switch(order){
				...
				case 2:fprintf(fin,"%d\n",order); delete_min() ;break;
				case 3:fprintf(fin,"%d ",order);x=rand()%size+1;fprintf(fin,"%d\n",x);delete_i(x);break;
				case 4:fprintf(fin,"%d ",order);p=rand()%size+1;x=rand()%Heap[p];fprintf(fin,"%d ",p);fprintf(fin,"%d\n",x);decrease_value(p, x);break;
			}
		} 
		fclose(fin);
