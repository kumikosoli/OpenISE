...
// 使用递归的方法实现路径搜索
   Status PathSearch(BiTree T, TElemType target, BiTree path[], int depth)
{
    if (T == NULL) {
        return FALSE;
    }
    ...
    if (PathSearch(T->lchild, target, path, depth + 1)) {
        path[depth] = T; // 将当前节点添加到路径中
        return OK;
    }
    if (PathSearch(T->rchild, target, path, depth + 1)) {
        path[depth] = T; // 将当前节点添加到路径中
        return OK;
    }
    return FALSE;
...
int main()
{
    ...
    // 调用路径搜索函数
    if (PathSearch(T, target, path, 0)) {
        printf("路径为：");
        for (int i = 0; i <= DeepTree(T); i++) {
