...
public class CbsTable extends JFrame {
	...
	public void myShow(String str) {
		...
		List<Cbs> myl = new CbsDao().getAll(str);
		for (Cbs fk : myl) {// 遍历集合
			// 将数据增加到表模式中
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (rt >= 0) {// 如果选中了
					...
					if (t == 0) {
						...
						Cbs fk = new CbsDao().getOne(tid);
						new UpdateCbs(CbsTable.this, fk);
					}
				} else {
